...
def retrieve_data():
    ...
    sanitary_total = list(current_user['How many sanitary products do you want to donate?'])[0]
    st.write(f"{sanitary_total} sanitary products")

    housing_total= list(current_user['How many housing goods do you want to donate?'])[0]
    st.write(f"{housing_total} housing goods")

    clothes_total = list(current_user['How many clothes/blankets do you want to donate?'])[0]
    st.write(f"{clothes_total} clothes/blankets")

    sleeping_total = list(current_user['How many sleeping mats do you want to donate?'])[0]
    st.write(f"{sleeping_total} sleeping mats")

    money_total = list(current_user['donated_amount'])[0]
